...
//M:start
static int do_gsm_call1( ControlClient  client, char*  args );
static int do_gsm_call2( ControlClient  client, char*  args );
static int do_gsm_reject( ControlClient  client, char*  args );
static int do_gsm_rejected( ControlClient  client, char*  args );
static int do_gsm_held( ControlClient  client, char*  args );
static int do_gsm_accepted( ControlClient  client, char*  args );
static int do_gsm_canceled( ControlClient  client, char*  args );
static int do_sms_send1( ControlClient  client, char*  args );
static int do_sms_send2( ControlClient  client, char*  args );
static int do_sms_sendpdu1( ControlClient  client, char*  args );
static int do_sms_sendpdu2( ControlClient  client, char*  args );
static int do_accelerometer_update( ControlClient  client, char*  args );
static int do_magnetic_field_sensor_update( ControlClient  client, char*  args );
static int do_orientation_sensor_update( ControlClient  client, char*  args );
static int do_temperature_sensor_update( ControlClient  client, char*  args );
static int do_sms_send_all( ControlClient  client, char*  args ,int sms);
static int do_sms_sendpdu_all( ControlClient  client, char*  args ,int sms);
//M:end
...
{
//M:start
    int ret =  do_gsm_call1(client,args);
    return ret;
//M:end
}
...
do_gsm_cancel( ControlClient  client, char*  args )
{
    if (!args) {
//M:start
        control_write( client, "KO: missing argument, try 'gsm call/call1/call2 <phonenumber>'\r\n" );
//M:end
        return -1;
    }
...
//M:start
    if ( amodem_cancel_call( android_modem, args ) < 0 ) {
//M:end
...
//M:start
    if ( amodem_hold_call( android_modem, args, A_CALL_HELD ) < 0 ) {
//M:end
        return -1;
    }
    return 0;
...
do_gsm_accept( ControlClient  client, char*  args )
{
    ...
    if ( amodem_accept_call( android_modem, args, A_CALL_ACTIVE ) < 0 ) {
...
//M:start
    { "hold", "change the state of an call to 'held'",
    "'gsm hold <remoteNumber>' change the state of a call to 'hold'. this is only possible\r\n"
    "if the call in the 'hold' or 'active' state\r\n", NULL,
    do_gsm_hold, NULL },
//M:end

    { "accept", "change the state of an outbound call to 'active'",
    "'gsm accept <remoteNumber>' change the state of a call to 'active'. this is only possible\r\n"
    "if the call is in the 'Incoming' state\r\n", NULL,
    do_gsm_accept, NULL },
...
//M:start
    { "call1", "create inbound phone call",
    "'gsm call1 <phonenumber>' allows you to simulate a new inbound call\r\n", NULL,
    do_gsm_call1, NULL },
    
    { "call2", "create inbound phone call",
    "'gsm call2 <phonenumber>' allows you to simulate a new inbound call\r\n", NULL,
    do_gsm_call2, NULL },
    
    { "reject", "close waiting outbound call as reject",
    "'gsm reject <remoteNumber>' closes an outbound call, reporting\r\n"
    "the remote phone as reject. only possible if the call is 'waiting'.\r\n", NULL,
    do_gsm_reject, NULL },

    { "rejected", "close waiting outbound call as rejected",
    "'gsm rejected <remoteNumber>' closes an outbound call, reporting\r\n"
    "the remote phone as rejected. only possible if the call is 'waiting'.\r\n", NULL,
    do_gsm_rejected, NULL },
    
    { "held", "change the state of an call to 'hold'",
    "'gsm held <remoteNumber>' change the state of a call to 'hold'. this is only possible\r\n"
    "if the call in the 'held' or 'active' held\r\n", NULL,
    do_gsm_held, NULL },
    
    { "accepted", "change the state of an outbound call to 'active'",
    "'gsm accepted <remoteNumber>' change the state of a call to 'active'. this is only possible\r\n"
    "if the call is in the 'Incoming' state\r\n", NULL,
    do_gsm_accepted, NULL },

    { "canceled", "disconnect an inbound or outbound phone call",
    "'gsm canceled <phonenumber>' allows you to simulate the end of an inbound or outbound call\r\n", NULL,
    do_gsm_canceled, NULL },
//M:end
    { NULL, NULL, NULL, NULL, NULL, NULL }
};
...
{
    D(( "[Emu] do sms send"));
    return do_sms_send_all( client,   args,1);
}

static int
do_sms_sendpdu( ControlClient  client, char*  args )
{
    return do_sms_sendpdu_all( client,  args, 1);
}
...
//M:start
    { "send1", "send inbound SMS text message to sim1",
    "'sms send1 <phonenumber> <message>' allows you to simulate a new inbound sms message to sim1\r\n", NULL,
    do_sms_send1, NULL },
    
    { "send2", "send inbound SMS text message to sim2",
    "'sms send2 <phonenumber> <message>' allows you to simulate a new inbound sms message to sim2\r\n", NULL,
    do_sms_send2, NULL },

    { "pdu1", "send inbound SMS PDU",
    "'sms pdu <hexstring>' allows you to simulate a new inbound sms PDU\r\n"
    "(used internally when one emulator sends SMS messages to another instance).\r\n"
    "you probably don't want to play with this at all\r\n", NULL,
    do_sms_sendpdu1, NULL },
    
    { "pdu2", "send inbound SMS PDU",
    "'sms pdu <hexstring>' allows you to simulate a new inbound sms PDU\r\n"
    "(used internally when one emulator sends SMS messages to another instance).\r\n"
    "you probably don't want to play with this at all\r\n", NULL,
    do_sms_sendpdu2, NULL },
//M:end
...
static int
do_avd_target( ControlClient  client, char*  args )
{
    control_write(client, "%s\r\n", avdInfo_getContentTarget(android_avdInfo));
    return 0;
}
...
static const CommandDefRec  vm_commands[] =
{
	...
	// Add for device target infromation.
	{ "target", "query virtual device target",
	"'avd target' will return the target of this virtual device\r\n",
	NULL, do_avd_target, NULL },
...
static int
do_gsm_call_all( ControlClient  client, char*  args,int sims )
{
/* check that we have a phone number made of digits */
    if (!args) {
        control_write( client, "KO: missing argument, try 'gsm call/call1/call2 <phonenumber>'\r\n" );
    ...
    amodem_gemini_set_sims( android_modem, sims);
    amodem_add_inbound_call( android_modem, args );
    return 0;
}

static int
do_gsm_call1( ControlClient  client, char*  args )
{
    ...
    return -1;
     ...
     if (amodem_gemini_get_sims(android_modem) < 0){
	control_write( client, "KO: the another sim is working." );
	return -1;
     }	

	if( A_RADIO_STATE_OFF == amodem_get_radio_state( android_modem)
		||A_RADIO_SIM2__ONLY_STATE_ON==  amodem_get_radio_state( android_modem)){
		control_write( client, "KO: sim1 is closed.\r\n" );
		return -1;
	}
		
     return do_gsm_call_all(client,args,1);
}

static int
do_gsm_call2( ControlClient  client, char*  args )
{
	    if (!android_modem) {
        control_write( client, "KO: modem emulation not running\r\n" );
        return -1;
	...
	if( A_RADIO_STATE_OFF == amodem_get_radio_state( android_modem)
		||A_RADIO_SIM1_ONLY_STATE_ON == amodem_get_radio_state( android_modem)){
		control_write( client, "KO: sim2 is closed.\r\n" );
		return -1;
	}
		
       return do_gsm_call_all(client,args,2);
}

static int
do_gsm_canceled( ControlClient  client, char*  args )
{
    if (!args) {
        control_write( client, "KO: missing argument, try 'gsm call/call1/call2 <phonenumber>'\r\n" );
        return -1;
    ...
    if ( amodem_canceled_call( android_modem, args ) < 0 ) {
        control_write( client, "KO: could not cancel this number\r\n" );
        return -1;
    }
    return 0;
...
static int
do_gsm_reject( ControlClient  client, char*  args )
{
    ACall  call;
    ...
    if (call == NULL || call->dir != A_CALL_INBOUND) {
        control_write( client, "KO: no current outbound call to number '%s' (call %p)\r\n", args, call );
        return -1;
    }
    if ( amodem_reject_call( android_modem, args ) < 0 ) {
        control_write( client, "KO: could not cancel this number\r\n" );
        return -1;
    }
    return 0;
...
static int
do_gsm_rejected( ControlClient  client, char*  args )
{
    ACall  call;

    if (!args) {
        control_write( client, "KO: missing argument, try 'gsm rejected <phonenumber>'\r\n" );
        return -1;
    }
    call = amodem_find_call_by_number( android_modem, args );
    ...
    if ( amodem_rejected_call( android_modem, args ) < 0 ) {
        control_write( client, "KO: could not cancel this number\r\n" );
        return -1;
    }
    return 0;
...
static int
do_gsm_held( ControlClient  client, char*  args )
{
    ACall  call;
    ...
    if ( amodem_held_call( android_modem, args ) < 0 ) {
...
static int
do_gsm_accepted( ControlClient  client, char*  args )
{
    ACall  call;
    ...
    if ( amodem_accepted_call( android_modem, args, A_CALL_ACTIVE ) < 0 ) {
        control_write( client, "KO: could not activate this call\r\n" );
        return -1;
    }
    return 0;
...
static int
do_sms_send_all( ControlClient  client, char*  args ,int sms)
{
    D(( "[Emu] do sms send all" ));
    char*          p;
    int            textlen;
    ...
    amodem_gemini_set_sms( android_modem, sms);
...
static int
do_sms_send2( ControlClient  client, char*  args )
{
	return do_sms_send_all( client,   args , 2);
}

static int
do_sms_send1( ControlClient  client, char*  args )
{
	return do_sms_send_all( client,   args , 1);
}

static int
do_sms_sendpdu_all( ControlClient  client, char*  args,int sim )
{
    SmsPDU  pdu;
    D(( "[Emu] do sms sendpdu all" ));;
    ...
    amodem_gemini_set_sms( android_modem, sim);
...
static int
do_sms_sendpdu1( ControlClient  client, char*  args )
{
    return do_sms_sendpdu_all( client,  args, 1);
}

static int
do_sms_sendpdu2( ControlClient  client, char*  args )
{
    return do_sms_sendpdu_all( client,  args, 2);
}
//M:end
